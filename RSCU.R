...
DESCRIPTION: This is the second program script of entire data analysis, for MAGs data codon-analysis.
"
...
source('Data_Analysis_FUNCTIONS_MAGsAanlysis.R')
...
# taxa_df <- bins_df %>% select(sample_id, Sample_type, phyla)
myMAGs <- list.files(path = './ffn/')
# Prepare empty RSCU matrices
rscu_genome_all_bins <- matrix(nrow = 64, ncol = length(myMAGs))
rscu_HiExp_all_bins  <- matrix(nrow = 64, ncol = length(myMAGs))
colnames(rscu_genome_all_bins) <- gsub(pattern = '\\.fasta.ffn', replacement = '', x = basename(myMAGs))
colnames(rscu_HiExp_all_bins)  <- gsub(pattern = '\\.fasta.ffn', replacement = '', x = basename(myMAGs))
##########################################
...
n <- 0
pb <- txtProgressBar(min = 0, max = length(myMAGs), char = "#", style = 3)
tic <- Sys.time()
print("STAGE 1:: Codon Usage Analysis ongoing...")
for(k in 1:length(myMAGs)){
  my_seq <- myMAGs[k]
  my_seq_fasta <- read.fasta(file = paste('./ffn/', my_seq, sep = ''), seqtype = 'DNA')
  strain_name <- gsub(pattern = '\\.fasta.ffn', replacement = '', x = my_seq)
  ...
  # Attaching the RSCU columns of each bin to the "rscu_all_bins" matrix
  rscu_genome_all_bins[,k] <- codons_dataset$rscu_genome
  rscu_HiExp_all_bins[,k] <-  codons_dataset$rscu_HiExp
  ...
  # Saving STATS
  stats_temp <- data.frame(BIN = strain_name, 
  ...
  codons_dataset$BIN <- strain_name
...
if (length(myMAGs) > 2){
  ...
  # rownames(bins_df) <- paste(bins_df$sample_id, "-", bins_df$phyla, sep = "")
  # RSCU genome matrix of all MAGs (Bins)
  row.names(rscu_genome_all_bins) <- codons_dataset$both_names
  write.table(x = rscu_genome_all_bins, file = paste(RESULT_dir, '/RSCU_genome_all_bins.txt', sep=""), quote = TRUE, sep = '\t', row.names = TRUE)
  rscu_genome_all_bins <- as.matrix(read.delim(file = paste(RESULT_dir, '/RSCU_genome_all_bins.txt', sep=""), sep = '\t', header = T))
  # colnames(rscu_genome_all_bins) <- paste(colnames(rscu_genome_all_bins), "-", as.data.frame(taxa_df)[match(colnames(rscu_genome_all_bins), taxa_df$sample_id), "phyla"], sep = "")
  ...
  temp_codons <- str_sub(rownames(rscu_genome_all_bins), start = 2, end=5)
  codons_metrics <- data.frame(codons=rownames(rscu_genome_all_bins), 
  ...
  # RSCU HiExp matrix of all MAGs (Bins)
  rownames(rscu_HiExp_all_bins) <- codons_dataset$both_names
  write.table(x = rscu_HiExp_all_bins, file = paste(RESULT_dir, '/RSCU_HiExp_all_bins.txt', sep=""), quote = TRUE, sep = '\t', row.names = TRUE)
  rscu_HiExp_all_bins <- as.matrix(read.delim(file = paste(RESULT_dir, '/RSCU_HiExp_all_bins.txt', sep=""), sep = '\t', header = T))
  # colnames(rscu_HiExp_all_bins) <- paste(colnames(rscu_HiExp_all_bins), "-", as.data.frame(taxa_df)[match(colnames(rscu_HiExp_all_bins), taxa_df$sample_id), "phyla"], sep = "")
  
  # Plot heatmap for HiExp data
  pdf(file = paste(RESULT_dir, '/03_HEATMAP_RSCU_HiExp.pdf', sep=""), width = 16, height = 8)
  pheatmap(rscu_HiExp_all_bins,
           fontsize_row = 6, 
           col= my_palette, 
           breaks = col_breaks, 
           annotation_row = annotation_row, 
  ...
  clust_rscu_genome_all_bins <- t(rscu_genome_all_bins[c(-30,-59),]) # Removing M-ATG and  W-TGG codons
  d <- dist(clust_rscu_genome_all_bins)
  ...
  clust_rscu_HiExp_all_bins <- t(rscu_HiExp_all_bins[c(-30,-59),]) # Removing M-ATG and  W-TGG codons
  d2 <- dist(clust_rscu_HiExp_all_bins)
